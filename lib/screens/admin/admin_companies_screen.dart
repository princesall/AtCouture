  ...
  Widget build(BuildContext context) {
    ...
    if (companies.isEmpty) {
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.apartment_outlined, color: c.onSurfaceVariant, size: 48),
        const SizedBox(height: 12),
        Text('Aucune entreprise multi-ateliers', style: AppTextStyles.bodySm.copyWith(color: c.onSurfaceVariant)),
      ]));
    }
    ...
    return ListView(
      ...
      children: [
        Text('Entreprises', style: AppTextStyles.titleMd.copyWith(color: c.primary)),
        const SizedBox(height: 4),
        Text('Comptes plan Entreprise — vue lecture seule de la structure', style: AppTextStyles.bodySm.copyWith(color: c.onSurfaceVariant)),
...
class _CompanyCardState extends State<_CompanyCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final ateliers = CompanyService.instance.ateliersOfCompany(company.ownerId as String);
    final stats = CompanyService.instance.companyStats(company.ownerId as String);
    final c = context.colors;
          ...
          child: Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(company.name as String, style: AppTextStyles.titleSm, maxLines: 1, overflow: TextOverflow.ellipsis),
                Text('Propriétaire : ${company.ownerName}', style: AppTextStyles.bodySm.copyWith(color: c.onSurfaceVariant)),
                const SizedBox(height: 6),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(gradient: c.goldGradient, borderRadius: BorderRadius.circular(999)),
                  child: Text('PLAN ENTREPRISE', style: AppTextStyles.labelXs.copyWith(color: c.onTertiary)),
                ),
              ])),
      ...
      child: Column(children: [
        ...
        Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.payments_rounded, color: c.onTertiary, size: 16),
            const SizedBox(width: 6),
            Text('${Formatters.formatCurrency(stats.totalRevenue as int)} ${AppConstants.currency} générés au total', style: AppTextStyles.bodySm.copyWith(color: c.onTertiary, fontWeight: FontWeight.w700)),
          ]),
        ),
        const SizedBox(height: 14),
        Align(alignment: Alignment.centerLeft, child: Text('STRUCTURE DES ATELIERS', style: AppTextStyles.labelCaps.copyWith(color: c.onSurfaceVariant))),
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('Chef : ${a.headStylistName}', style: AppTextStyles.labelXs.copyWith(color: c.onSurfaceVariant)),
              ])),
              Text('${(a.tailorIds as List).length} couturiers', style: AppTextStyles.labelXs.copyWith(color: c.primary)),
            ]),
          ),
        )),
      ]),
...
class _StatChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Expanded(child: Container(
      ...
      decoration: BoxDecoration(color: c.primary.withValues(alpha: 0.06), borderRadius: BorderRadius.circular(10)),
      child: Column(children: [
        Icon(icon, size: 14, color: c.primary),
        const SizedBox(height: 3),
        Text(value, style: AppTextStyles.titleSm.copyWith(color: c.primary, fontSize: 13)),
        Text(label, style: AppTextStyles.labelXs.copyWith(fontSize: 8, color: c.onSurfaceVariant)),
      ]),
    ));
  }
}
